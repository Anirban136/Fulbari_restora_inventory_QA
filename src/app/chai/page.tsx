                    ...
                    {localStock.length === 0 ? (
                      ...
                      localStock.map(stock => (
                        <li key={stock.id} className="flex justify-between items-center p-3 rounded-xl border border-white/5 bg-white/5">
                          ...
                          <div className="font-black text-blue-400 bg-blue-500/10 px-2.5 py-1.5 rounded-lg text-sm flex flex-col items-end">
                            <span>
                              {stock.quantity} <span className="text-[9px] ml-0.5 opacity-70 uppercase font-black">{stock.Item.piecesPerBox ? 'pcs' : stock.Item.unit}</span>
                            </span>
                            {stock.Item.piecesPerBox && (
                              <span className="text-[8px] opacity-60 uppercase font-black tracking-tighter -mt-0.5">
                        ...
                        <li key={log.id} className="flex items-center justify-between p-5 rounded-2xl border border-white/5 bg-white/5 hover:bg-white/10 hover:border-white/20 transition-all group shadow-sm">
                          ...
                          <div className="text-right">
                            <div className="flex flex-col items-end">
                              <span className="font-black text-sky-400 bg-sky-500/10 border border-sky-500/20 px-4 py-2 rounded-xl text-lg lg:text-xl drop-shadow-[0_0_5px_rgba(14,165,233,0.3)]">
                              ...
                              {log.Item.piecesPerBox && (
                                <span className="text-[10px] text-sky-500/50 font-black uppercase tracking-tighter mt-1">
                            ...
                            <div className="mt-3 flex flex-wrap items-center gap-2">
                              ...
                              {(tab.status === "CLOSED" || tab.status === "CANCELLED") && (
                                <form action={reopenTab.bind(null, tab.id)}>
                                  <Button type="submit" variant="ghost" size="sm" className="h-8 text-xs font-bold text-slate-400 hover:text-blue-400 hover:bg-blue-500/10 transition-colors border border-transparent hover:border-blue-500/20">
