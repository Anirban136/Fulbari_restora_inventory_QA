...
import { Edit, Package, Check, RefreshCcw } from "lucide-react"
import {
  Dialog,
  DialogContent,
  DialogDescription,
      ...
      <DialogContent className="sm:max-w-[500px] bg-zinc-950/95 backdrop-blur-3xl border-white/10 rounded-[3rem] shadow-2xl p-0 overflow-hidden">
        ...
        <div className="p-10">
          <DialogHeader className="mb-10">
            ...
            <DialogTitle className="text-4xl font-black text-white tracking-tighter uppercase leading-none">Node Config</DialogTitle>
            <DialogDescription className="text-slate-400 font-medium mt-4 tracking-tight leading-relaxed">
              Reconfiguring item: <span className="text-amber-500 font-black uppercase">{item.name}</span> in the central repository.
            </DialogDescription>
          </DialogHeader>
          ...
          <form action={handleSubmit} className="space-y-8">
            ...
            <div className="space-y-3">
              <Label htmlFor={`edit_name_${item.id}`} className="text-[10px] font-black text-muted-foreground uppercase tracking-[0.3em] ml-1">Identifier</Label>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-3">
                <Label htmlFor={`edit_category_${item.id}`} className="text-[10px] font-black text-muted-foreground uppercase tracking-[0.3em] ml-1">Class</Label>
                <CategoryCombobox 
                  name="category" 
              ...
              <div className="space-y-3">
                <Label htmlFor={`edit_unit_${item.id}`} className="text-[10px] font-black text-muted-foreground uppercase tracking-[0.3em] ml-1">Metric</Label>
                <select 
                  name="unit" 
            ...
            <div className="grid grid-cols-2 gap-6">
              <div className="space-y-3">
                <Label htmlFor={`edit_cost_${item.id}`} className="text-[10px] font-black text-muted-foreground uppercase tracking-[0.3em] ml-1">Input (₹)</Label>
              ...
              <div className="space-y-3">
                <Label htmlFor={`edit_sell_${item.id}`} className="text-[10px] font-black text-emerald-500 uppercase tracking-[0.3em] ml-1">Output (₹)</Label>
            ...
            <div className="space-y-3 bg-primary/5 p-5 rounded-3xl border border-primary/10">
              <Label htmlFor={`edit_stock_${item.id}`} className="text-[10px] font-black text-primary uppercase tracking-[0.3em] ml-1 flex items-center justify-between">
                <span>MANUAL PARITY SYNC</span>
                <span className="opacity-40 text-[8px] tracking-normal lowercase">(Direct Override)</span>
              </Label>
              <div className="relative">
                <Input id={`edit_stock_${item.id}`} name="currentStock" type="number" step="0.01" defaultValue={item.currentStock} required className="h-16 bg-black/40 border-primary/30 text-primary text-3xl font-black rounded-2xl tracking-tighter pl-6 focus-visible:ring-primary/40 shadow-2xl" />
                <span className="absolute right-6 top-1/2 -translate-y-1/2 text-[10px] font-black text-primary/40 uppercase tracking-widest">{item.unit}</span>
              </div>
            </div>

            <div className={`grid gap-6 transition-all duration-500 ${showPiecesPerBox ? 'grid-cols-2' : 'grid-cols-1'}`}>
              <div className="space-y-3">
                <Label htmlFor={`edit_min_stock_${item.id}`} className="text-[10px] font-black text-amber-500 uppercase tracking-[0.3em] ml-1">Redline Alert</Label>
              ...
              {showPiecesPerBox && (
                <div className="space-y-3 animate-in fade-in slide-in-from-right-4 duration-500">
                  <Label htmlFor={`edit_pieces_per_box_${item.id}`} className="text-[10px] font-black text-blue-400 uppercase tracking-[0.3em] ml-1">Node Density</Label>
            ...
            <div className="pt-4 pb-4">
              <Button type="submit" className="w-full h-16 text-sm font-black uppercase tracking-[0.4em] bg-white text-black hover:bg-slate-100 rounded-2xl shadow-2xl transition-all active:scale-95 flex items-center justify-center gap-3">
                Sync Changes <RefreshCcw className="w-4 h-4" />
              </Button>
            </div>
          </form>
        </div>
